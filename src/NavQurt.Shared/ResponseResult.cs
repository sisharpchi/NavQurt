...
public record ResponseResult<T, TError> : ResponseResult
{
    ...
    public static ResponseResult<T, TError> CreateError(string error, TError? errorData) => new() { Error = error, ErrorData = errorData };
    public static ResponseResult<T, TError> CreateError(string error, int errorCode, TError? errorData) => new() { Error = error, ErrorCode = errorCode, ErrorData = errorData };
}
